...
int main() {
    ...
    while(completed < n) {
        ...
        int executed_this_cycle = 0;
        for(int i = 0; i < n; i++) {
            if(at[i] <= time && rt[i] > 0 && pr[i] == highest_pr) {
                executed_this_cycle = 1;
                seq[index] = i;
                ...
                int check_new_pr = 0;
                for(int j = 0; j < n; j++) {
                    if(at[j] <= time && rt[j] > 0 && pr[j] < highest_pr) {
                        check_new_pr = 1; break;
                    }
                }
                if(check_new_pr) break; 
            }
        }

        if(executed_this_cycle == 0) {
            if (index > 0 && seq[index-1] == -1) {
                time++;
                timeline[index] = time;
            } else {
                seq[index] = -1;
    ...
    printf("\nGantt Chart:\n|");
    for(int i = 0; i < index; i++) {
        if(seq[i] == -1) printf(" IDLE |");
        else printf("  P%d  |", seq[i] + 1);
    }
    printf("\n");
    for(int i = 0; i <= index; i++) printf("%-7d", timeline[i]);
    printf("\n");
